...
class FaceService:
    ...
    def extract_face_features(self, image: np.ndarray, face_box: Tuple[int, int, int, int]) -> np.ndarray:
        """Extract simple face features using histogram and basic descriptors"""
        x, y, w, h = face_box
        ...
        lbp_features = self._compute_lbp_features(resized_face)
        features.extend(lbp_features)
        
        # 3. Edge features
        edges = cv2.Canny(resized_face, 50, 150)
        edge_hist = cv2.calcHist([edges], [0], None, [256], [0, 256])
        features.extend(edge_hist.flatten().tolist())
