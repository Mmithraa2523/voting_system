...
class Voter(db.Model):
    ...
    auth_events = db.relationship('AuthEvent', backref='voter', lazy=True, cascade='all')
...
class VoterFace(db.Model):
    ...
    def set_encoding(self, encoding_array):
        ...
        # Handle list
        elif isinstance(encoding_array, list):
            self.encoding = json.dumps(encoding_array)
        # Handle tuple - convert to list first
        elif isinstance(encoding_array, tuple):
            ...
            try:
                ...
                raise ValueError(f"Cannot convert encoding of type {type(encoding_array)} to JSON")
